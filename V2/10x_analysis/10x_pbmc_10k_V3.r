...
# read data 
raw = Read10X_MPM("data/10x_data/10x_pbmc10k_V3/raw_feature_bc_matrix/")
...
#   geom_vline(xintercept = c(2.7, 1.2 ),   linetype = "dashed") + 
...
# p1
# p2 = ggplot(s@meta.data %>% filter(log10(nUMI + 1) > 3), aes(x = log10(nUMI + 1 ) )) +
#   theme_bw() + 
#   theme(text = element_text(size = 8)) + 
#   geom_density(fill = "#3e8ede") + 
