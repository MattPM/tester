...
# neg_adt = readRDS("data/V2_Data/neg_drops_batch1_batch2_list.rds")
# neg_adt = lapply(neg_adt, t ) 
# neg_adt_sub = readRDS("data/V2_Data/negative_drop_subset_batch1_2_Seurat2.4.rds")
# negadt_sub = lapply(neg_adt_sub, function(x){x@assay$CITE@raw.data %>% as.matrix %>% as.data.frame()})
  ...
  x = x %>% 
    mutate(prot_class = 
             if_else(protein %in% bcp, "bcell_protein", 
              ...
              if_else(protein %in% mcp, "myeloid_protein", 
                      false = "other"))))
